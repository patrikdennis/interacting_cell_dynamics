...
# Now with multiple output modes for a complete segmentation and feature-extraction pipeline.
# """
# import os
# import argparse
# import numpy as np
...
#     pil_kwargs = {}
#     if out_filename.lower().endswith((".tif", ".tiff")):
...
#         segmented_path = os.path.join(path_segmented, out_filename)
#         fig.savefig(segmented_path, dpi=dpi, bbox_inches="tight", pad_inches=0, pil_kwargs=pil_kwargs)
...
#         features_path = os.path.join(path_features, out_filename)
#         cv2.imwrite(features_path, feature_image, [cv2.IMWRITE_TIFF_COMPRESSION, 5])

#         buf = io.BytesIO()
#         fig.savefig(buf, format='png', dpi=dpi, bbox_inches="tight", pad_inches=0)
#         buf.seek(0)
#         img_contour_np = np.frombuffer(buf.getvalue(), dtype=np.uint8)
#         img_contour_cv = cv2.imdecode(img_contour_np, cv2.IMREAD_COLOR)
#         buf.close()
        
#         img_contour_cv = cv2.resize(img_contour_cv, (W0, H0))

#         for cX, cY in centroids:
#             half_size = square_size // 2
#             x1, y1 = cX - half_size, cY - half_size
#             x2, y2 = x1 + square_size, y1 + square_size
#             cv2.rectangle(img_contour_cv, (x1, y1), (x2, y2), (255, 255, 255), thickness=cv2.FILLED)
#         overlay_path = os.path.join(path_overlay, out_filename)
#         cv2.imwrite(overlay_path, img_contour_cv, [cv2.IMWRITE_TIFF_COMPRESSION, 5])

#     else:
#         segmented_path = os.path.join(out_dir, out_filename)
#         fig.savefig(segmented_path, dpi=dpi, bbox_inches="tight", pad_inches=0, pil_kwargs=pil_kwargs)
...
def draw_contours(
    ...
    for P in kept:
        contour = P[:, ::-1].astype(np.int32)
        epsilon = approximation_level * cv2.arcLength(contour, True)
        approx_contour = cv2.approxPolyDP(contour, epsilon, True)
        refined_contours.append(approx_contour)
    ...
    # This is the new, robust way to generate an image from the plot
    def render_figure_to_cv_image(fig, width, height):
        buf = io.BytesIO()
        fig.savefig(buf, format='png', dpi=dpi, bbox_inches="tight", pad_inches=0)
        buf.seek(0)
        ...
        buf.close()
        # Crop to exact dimensions to remove any residual padding
        return img_cv[:height, :width]

    fig, ax = plt.subplots(figsize=(fig_w, fig_h), dpi=dpi)
    ax.imshow(I0, cmap="gray")
    color_cycle = cycle(BRIGHT_COLORS)
    for approx_contour in refined_contours:
        ax.plot(approx_contour[:, 0, 0], approx_contour[:, 0, 1], linewidth=0.9, color=next(color_cycle))
    ax.set_axis_off()
    plt.tight_layout(pad=0)

    # Convert the Matplotlib figure to an OpenCV image
    segmented_image_cv = render_figure_to_cv_image(fig, W0, H0)
    plt.close(fig)
    ...
    # All saving is now done with cv2.imwrite for perfect dimensions
    tiff_compression = [cv2.IMWRITE_TIFF_COMPRESSION, 5] # 5 = LZW
    ...
    if mode == 'all':
        ...
        # 3. Save the overlay image (squares on segmented)
        overlay_image = segmented_image_cv.copy()
        for cX, cY in centroids:
            half_size = square_size // 2
            x1, y1 = cX - half_size, cY - half_size
            x2, y2 = x1 + square_size, y1 + square_size
            cv2.rectangle(overlay_image, (x1, y1), (x2, y2), (255, 255, 255), thickness=cv2.FILLED)
